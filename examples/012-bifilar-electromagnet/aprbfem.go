// -*- compile-command: "go run aprbfem.go -n 12"; -*-
...
func (m *arBifilarElectromagnet) coilSquareFace(wireNum int, radius, trimStartAngle, trimEndAngle float64) {
	...
	for ; angle <= endAngle-delta; angle += delta {
		m.coilWireSegment(angle, angle+delta, ri, ro)
	}
}

func (m *arBifilarElectromagnet) coilWireSegment(a1, a2, ri, ro float64) {
	z1 := a1 / (2.0 * math.Pi)
	z2 := a2 / (2.0 * math.Pi)
	...
	write(&n2o, p1uo, p2do, p1do)
	write(&n2o, p1uo, p2uo, p2do)
}
