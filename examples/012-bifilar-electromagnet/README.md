...
vec3 arBifilarElectromagnet(int numPairs,float innerRadius,float size,float gap,int numTurns,in vec3 xyz){
  ...
  vec3 xyz180Z=(vec4(xyz,1.)*xfm).xyz;
  
  for(int i=1;i<=numPairs;i++){
    metal1+=coilPlusConnectorWires(i,numPairs,inc,innerRadius,connectorRadius,size,size+2.*gap,nTurns,xyz);
    metal2+=coilPlusConnectorWires(i,numPairs,inc,innerRadius,connectorRadius,size,size+2.*gap,nTurns,xyz180Z);
  }
  metal1 = clamp(metal1, 0.0, 1.0);
  metal2 = clamp(metal2, 0.0, 1.0);
...
void mainModel4(out vec4 materials,in vec3 xyz){
  xyz.z+=60.;
  materials.xyz=arBifilarElectromagnet(10,3.,.85,.15,122,xyz);
}
```
